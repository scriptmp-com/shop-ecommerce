...
if (WPBakeryShortCode_VC_Tta_Section::$section_info) :
    ...
    foreach (WPBakeryShortCode_VC_Tta_Section::$section_info as $k => $v) :
        ...
        if (isset($v["section_nasa_icon"]) && trim($v["section_nasa_icon"]) !== '') :
            $v['add_icon'] = 'true';
            $custom_icon = true;
        endif;
        ...
        if (isset($v["section_nasa_img"]) && absint($v["section_nasa_img"])) :
            ...
            if ($img) :
                $icon_img = '<img src="' . esc_url($img[0]) . '" alt="' . esc_attr($v['title']) . '" width="' . absint($img[1]) . '" height="' . absint($img[2]) . '" />';
        ...
        if ($v['add_icon'] == 'true') :
            $icon = 'nasa-tab-icon ';
            $icon .= !$custom_icon ? 'vc_tta-icon ' . $v['i_icon_' . $v['i_type']] : $v["section_nasa_icon"];
            
            switch ($v['i_position']) :
                case 'right':
                    $icon .= ' padding-top-10';
                    $icon_img = $icon_img != '' ? '<span class="tab-icon-img nasa-block padding-top-10">' . $icon_img . '</span>' : '';
                    $title = $title . '<i class="' . $icon . '"></i>' . $icon_img;
                    break;
                case 'left':
                default :
                    $icon .= ' padding-bottom-10';
                    $icon_img = $icon_img != '' ? '<span class="tab-icon-img nasa-block padding-bottom-10">' . $icon_img . '</span>' : '';
                    $title = '<i class="' . $icon . '"></i>' . $icon_img . $title;
                    break;
            endswitch;
        endif;
        ...
        $content .= '<a href="javascript:void(0);" data-hash="#' . esc_attr($v['tab_id']) . '" data-index="nasa-section-' . esc_attr($v['tab_id']) . '" class="' . esc_attr($class_a_click) . '"' . (!empty($tab_color_text) ? ' style="' . implode(';', $tab_color_text) . '"' : '') . ' rel="nofollow">' . $title . '</a></li>';
    endforeach;
