...
if (!function_exists('elessi_static_cart_sidebar')) :
    function elessi_static_cart_sidebar() {
        ...
        <?php
        // echo '<div class="hidden-tag ns-after-mini-cart">';
        // do_action('nasa_mini_cart_cross_sell');
        // echo '</div>';
    }
endif;
 ...
 */
/* add_filter('woocommerce_add_to_cart_fragments', 'elessi_mini_cart_crosssells');
if (!function_exists('elessi_mini_cart_crosssells')) :
    function elessi_mini_cart_crosssells($fragments) {
        global $nasa_opt;
        
        if (isset($nasa_opt['event-after-add-to-cart']) && $nasa_opt['event-after-add-to-cart'] == 'sidebar' && isset($nasa_opt['mini_cart_crsells']) && $nasa_opt['mini_cart_crsells']) {
            // $fragments['.ns-after-mini-cart']
        }
...
if (!function_exists('elessi_link_style_off_canvas')) :
    function elessi_link_style_off_canvas() {
        ...
        echo !isset($nasa_opt['css_canvas']) || $nasa_opt['css_canvas'] == 'async' ? '<script type="text/template" id="nasa-style-off-canvas" data-link="' . esc_url(ELESSI_THEME_URI . '/assets/css/style-off-canvas.css') . '"></script>' : '';
        
        echo !isset($nasa_opt['css_canvas']) || $nasa_opt['css_canvas'] == 'async' ? '<script type="text/template" id="nasa-style-mobile-menu" data-link="' . esc_url(ELESSI_THEME_URI . '/assets/css/style-mobile-menu.css') . '"></script>' : '';
    }
endif;
